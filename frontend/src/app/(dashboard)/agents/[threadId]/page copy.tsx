...
import { ArrowDown, CheckCircle, CircleDashed, AlertTriangle, Info, File, ChevronRight } from 'lucide-react';
import {
  addUserMessage,
  startAgent,
  stopAgent,
...
export default function ThreadPage({ params }: { params: Promise<ThreadParams> }) {
  ...
  const handleSubmitMessage = useCallback(
    async (message: string, options?: { model_name?: string; enable_thinking?: boolean }) => {
      ...
      try {
        const results = await Promise.allSettled([addUserMessage(threadId, message), startAgent(threadId, options)]);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(([entry]) => setShowScrollButton(!entry?.isIntersecting), { root: messagesContainerRef.current, threshold: 0.1 });
  ...
  useEffect(() => {
    ...
    if (
      (streamHookStatus === 'completed' || streamHookStatus === 'stopped' || streamHookStatus === 'agent_not_running' || streamHookStatus === 'error') &&
        ...
        <div className="flex justify-between items-center">
          ...
          <div
            className={`px-2 py-0.5 rounded-full text-xs ${
              isSuccess ? 'bg-green-50 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-red-50 text-red-700 dark:bg-red-900 dark:text-red-300'
            }`}
          >
    ...
    assistantMessages.forEach((assistantMsg) => {
      ...
      if (resultMessage) {
        ...
        try {
          ...
          isSuccess = !(toolContent.includes('failed') || toolContent.includes('error') || toolContent.includes('failure'));
        } catch {}
  ...
  const handleToolClick = useCallback(
    (clickedAssistantMessageId: string | null, clickedToolName: string) => {
      ...
      const toolIndex = toolCalls.findIndex((tc) => {
        ...
        return tc.assistantCall?.content === assistantMessage.content && tc.toolResult?.content === toolMessage?.content;
      });
  ...
  if (!initialLoadCompleted.current || isLoading) {
    ...
    return (
      <div className="flex h-screen relative">
        ...
        {debugMode && <div className="fixed top-16 right-4 bg-amber-500 text-black text-xs px-2 py-1 rounded-md shadow-md z-50">Debug Mode</div>}
        <div onClick={test} className="absolute top-10 left-10 bg-cyan-200  px-5 py-2 rounded-md cursor-pointer z-50">
          test
        </div>
        <div
