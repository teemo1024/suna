...
import { extractPrimaryParam, getToolIcon, safeJsonParse } from '@/components/thread/utils';
...
export function renderMarkdownContent(
  ...
  if (debugMode) {
    return <pre className="text-xs font-mono whitespace-pre-wrap overflow-x-auto p-2 border border-border rounded-md bg-muted/30 text-foreground">{content}</pre>;
  }
    ...
    if (toolName === 'ask') {
      ...
      contentParts.push(
        ...
        >
          ...
          {paramDisplay && (
            <span className="ml-1 text-muted-foreground truncate max-w-[200px]" title={paramDisplay}>
              {paramDisplay}
            </span>
          )}
        </button>,
...
export const ThreadContent: React.FC<ThreadContentProps> = ({
  ...
  useEffect(() => {
    console.log('streamingTextContent', streamingTextContent);
  }, [streamingTextContent]);
  ...
  return (
    <>
        ...
        <div className="mx-auto max-w-3xl">
          {displayMessages.length === 0 && !streamingTextContent && !streamingToolCall && !streamingText && !currentToolCall && agentStatus === 'idle' ? (
            <div className="flex h-full items-center justify-center">
              <div className="text-center text-muted-foreground">{readOnly ? 'No messages to display.' : 'Send a message to start.'}</div>
            </div>
          ) : (
            <div className="space-y-8">
                      ...
                      return (
                        <div key={group.key} className="flex justify-end">
                          <div className="inline-flex max-w-[85%] rounded-xl bg-primary/10 px-4 py-3">
                            <pre className="text-xs font-mono whitespace-pre-wrap overflow-x-auto">{message.content}</pre>
                          </div>
                        </div>
                      );
                    }
                          ...
                          <div className="flex-1">
                            <div className="inline-flex max-w-[90%] rounded-lg bg-muted/5 px-4 py-3 text-sm">
                              <div className="space-y-2">
                                ...
                                {groupIndex === groupedMessages.length - 1 && !readOnly && true && (
                                  <div className="mt-2">
                                    {(() => {
                                      ...
                                      return (
                                        <>
                                          ...
                                          {showCursor && <span className="inline-block h-4 w-0.5 bg-primary ml-0.5 -mb-1 animate-pulse" />}
                                              ...
                                              {(() => {
                                                ...
                                                return (
                                                  <button className="inline-flex items-center gap-1.5 py-1 px-2.5 text-xs font-medium text-primary bg-primary/10 hover:bg-primary/20 rounded-md transition-colors cursor-pointer border border-primary/20">
                                                    ...
                                                    {paramDisplay && (
                                                      <span className="ml-1 text-primary/70 truncate max-w-[200px]" title={paramDisplay}>
                                                        {paramDisplay}
                                                      </span>
                                                    )}
                                                  </button>
              ...
              {(agentStatus === 'running' || agentStatus === 'connecting') && !readOnly && (messages.length === 0 || messages[messages.length - 1].type === 'user') && (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="inline-flex items-center gap-1.5 py-1.5 px-3 text-xs font-medium text-primary bg-primary/10 rounded-md border border-primary/20">
                        <CircleDashed className="h-3.5 w-3.5 text-primary flex-shrink-0 animate-spin animation-duration-2000" />
                        <span className="font-mono text-xs text-primary">{currentToolCall.name || 'Using Tool'}</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {showScrollButton && (
        <Button variant="outline" size="icon" className="fixed bottom-20 right-6 z-10 h-8 w-8 rounded-full shadow-md" onClick={() => scrollToBottom('smooth')}>
